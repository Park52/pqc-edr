...
// security_event 디코드 → 분류 파이프라인(prefilter → Haiku → Sonnet) → alert.
//
// LLM 선택: --mock-llm 이거나 ANTHROPIC_API_KEY 가 없으면 Mock, 아니면 Claude API.
//
//   analyzer --id analyzer --peer agent.pub [--port 9443] [--mock-llm] [--once]
...
static void serve_connection(int cfd, const Identity &id, LlmClient &llm) {
...
int main(int argc, char **argv) {
    ...
    int lfd = tcp_listen(o.port);
    fprintf(stderr, "[analyzer] 리슨: 127.0.0.1:%u (Ctrl-C 종료)\n", o.port);
