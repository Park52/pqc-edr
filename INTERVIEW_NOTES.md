...
성능: 벤치 기준 이벤트당 ~13–14µs로 execve 경로(fork+exec ~630µs)엔 +2%, connect 커널 경로엔 +31%지만 절대치가 같아 실 워크로드(초당 수백 이벤트)에선 CPU 1% 미만이고, ring buffer는 커널→유저 복사 1회에 reserve/commit이라 부분 이벤트가 나가지 않는다. 보안: eBPF 프로그램은 verifier가 종료·메모리 안전을 증명한 뒤에만 로드돼 커널 모듈보다 안전하고, root 없이 cap_bpf+cap_perfmon 최소권한으로 attach했다. 소비자(analyzer)가 느릴 때는 ring buffer 콜백이 유한 큐에 복사만 하고 송신 스레드가 보내므로 커널이 아니라 유저스페이스에서 드롭이 나고, 그 수를 세어 `AGENT_DROP` 통지로 알린다. 훅은 3개(execve, IPv4 connect, security_file_open)인데, 파일훅은 시스템 전체 open 마다 실행돼도 커널 안에서 f_mode·inode 로 강하게 걸러 open 당 ~0.3µs 이고 흥미로운 것만 ring buffer 에 올린다. 남은 한계는 kprobe/fentry 가 커널 심볼에 의존하는 점(CO-RE 로 레이아웃 변화는 흡수, 심볼 삭제는 못 막음)이다. ([docs/BENCHMARK.md §4](docs/BENCHMARK.md))

### 5. 실제 프로덕션에 올린다면 뭐가 부족한가? (솔직하게)
신뢰·세션: PKI 없이 사전공유 공개키 핀닝이라 키 배포·회전·폐기(revocation)가 없고, 세션 재개·키 갱신·재연결 백오프도 없어 링크가 끊기면 이벤트가 유실된다(백프레셔는 유한 큐로 드롭을 세고 알리지만 디스크 큐·재전송은 없다). 탐지: 파일훅(민감읽기·write→exec)과 계보(조상 교집합)로 초기 사각지대 2개(부모 분리 회피, cat /etc/shadow)는 닫았지만, 여전히 IPv6/UDP·권한상승(setuid)·커널모듈 로드는 못 보고, 룰·코릴레이션 임계는 튜닝 중인 휴리스틱이며(탐지 평가로 회귀를 게이트), execve 상대경로는 write→exec 매칭에서 놓칠 수 있고 root 공격자는 agent 자체를 끌 수 있다. 실측상 LLM 은 정상 개발 도구(gmake→sh·docker)에 ~9% 오탐이 있어 앵커가 아니라 triage 다. 운영: 핸드셰이크가 TLS 1.3 축약판이라 형식 검증(formal analysis)이 없고, 감사 로그·메트릭·알림 라우팅·멀티테넌시·HA 같은 서비스 요소가 전무하다.
